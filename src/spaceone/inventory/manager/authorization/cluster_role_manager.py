...
from spaceone.inventory.model.authorization.cluster_role.cloud_service import (
    ClusterRoleResource,
    ClusterRoleResponse,
)
from spaceone.inventory.model.authorization.cluster_role.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class ClusterRoleManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        cluster_role_conn: ClusterRoleConnector = self.locator.get_connector(
        ...
        for cluster_role in list_all_cluster_role:
            try:
                ...
                if raw_readonly.get("aggregation_rule", {}) is not None:
                    raw_data["aggregation_rule"][
                        "cluster_role_selectors"
                    ] = self._convert_cluster_role_selectors(
                        raw_readonly.get("aggregation_rule", {}).get(
                            "cluster_role_selectors", []
                        )
                    )
                ...
                _LOGGER.debug(
                    f"cluster_role_data => {cluster_role_data.to_primitive()}"
                )
                ...
                collected_cloud_services.append(
                    ClusterRoleResponse({"resource": cluster_role_resource})
                )
                ...
                error_response = self.generate_resource_error_response(
                    e, "Authorization", "ClusterRole", cluster_role_name
                )
    ...
    def _convert_cluster_role_selectors(self, origin_cluster_role_selectors) -> list:
        ...
        for cluster_role_selector in origin_cluster_role_selectors:
            convert_cluster_role_selectors.append(
                {
                    "match_expressions": cluster_role_selector.get(
        ...
        _LOGGER.debug(
            f"_convert_cluster_role_selectors => {convert_cluster_role_selectors}"
        )
