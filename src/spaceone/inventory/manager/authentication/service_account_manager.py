...
from spaceone.inventory.model.authentication.service_account.cloud_service import ServiceAccountResource, \
    ServiceAccountResponse
from spaceone.inventory.model.authentication.service_account.cloud_service_type import CLOUD_SERVICE_TYPES
...
class ServiceAccountManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        service_account_conn: ServiceAccountConnector = self.locator.get_connector(self.connector_name, **params)
        ...
        for service_account in list_all_service_account:
            try:
                ...
                ##################################
                collected_cloud_services.append(ServiceAccountResponse({'resource': service_account_resource}))

            except Exception as e:
                _LOGGER.error(f'[collect_cloud_service] => {e}', exc_info=True)
                # Pod name is key
                error_response = self.generate_resource_error_response(e, 'Authentication', 'ServiceAccount', service_account_name)
