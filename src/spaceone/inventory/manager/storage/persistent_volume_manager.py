...
from spaceone.inventory.model.storage.persistent_volume.cloud_service import (
    PersistentVolumeResource,
    PersistentVolumeResponse,
)
from spaceone.inventory.model.storage.persistent_volume.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class PersistentVolumeManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        persistent_volume_conn: PersistentVolumeConnector = self.locator.get_connector(
        ...
        for persistent_volume in list_all_persistent_volume:
            try:
                ...
                _LOGGER.debug(
                    f"persistent_volume_data => {persistent_volume_data.to_primitive()}"
                )
                ...
                collected_cloud_services.append(
                    PersistentVolumeResponse({"resource": persistent_volume_resource})
                )
                ...
                error_response = self.generate_resource_error_response(
                    e, "Storage", "PersistentVolume", persistent_volume_name
                )
