...
from spaceone.inventory.model.storage.storage_class.cloud_service import (
    StorageClassResource,
    StorageClassResponse,
)
from spaceone.inventory.model.storage.storage_class.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class StorageClassManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        storage_class_conn: StorageClassConnector = self.locator.get_connector(
        ...
        for storage_class in list_all_storage_class:
            try:
                ...
                _LOGGER.debug(
                    f"storage_class_data => {storage_class_data.to_primitive()}"
                )
                ...
                collected_cloud_services.append(
                    StorageClassResponse({"resource": storage_class_resource})
                )
                ...
                error_response = self.generate_resource_error_response(
                    e, "Storage", "StorageClass", storage_class_name
                )
