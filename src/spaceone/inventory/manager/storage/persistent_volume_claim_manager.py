...
from spaceone.inventory.model.storage.persistent_volume_claim.cloud_service import PersistentVolumeClaimResource, \
    PersistentVolumeClaimResponse
from spaceone.inventory.model.storage.persistent_volume_claim.cloud_service_type import CLOUD_SERVICE_TYPES
...
class PersistentVolumeClaimManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        pvc_conn: PersistentVolumeClaimConnector = self.locator.get_connector(self.connector_name, **params)
        ...
        for pvc in list_all_pvc:
            try:
                ...
                ##################################
                collected_cloud_services.append(PersistentVolumeClaimResponse({'resource': pvc_resource}))

            except Exception as e:
                _LOGGER.error(f'[collect_cloud_service] => {e}', exc_info=True)
                # Pod name is key
                error_response = self.generate_resource_error_response(e, 'Storage', 'PersistentVolumeClaim', pvc_name)
