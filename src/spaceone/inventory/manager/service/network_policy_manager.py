...
from spaceone.inventory.model.service.network_policy.cloud_service import (
    NetworkPolicyResource,
    NetworkPolicyResponse,
)
from spaceone.inventory.model.service.network_policy.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class NetworkPolicyManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        network_policy_conn: NetworkPolicyConnector = self.locator.get_connector(
        ...
        for network_policy in list_all_network_policy:
            try:
                ...
                raw_data["ingress"]["from"][
                    "namespace_selector"
                ] = self.convert_labels_format(
                    raw_readonly.get("ingress", {})
                    .get("from", {})
                ...
                _LOGGER.debug(
                    f"network_policy_data => {network_policy_data.to_primitive()}"
                )
                ...
                collected_cloud_services.append(
                    NetworkPolicyResponse({"resource": network_policy_resource})
                )
                ...
                error_response = self.generate_resource_error_response(
                    e, "Service", "NetworkPolicy", network_policy_name
                )
