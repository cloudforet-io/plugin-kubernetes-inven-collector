...
from spaceone.inventory.model.cluster.cluster.cloud_service import (
    ClusterResource,
    ClusterResponse,
)
from spaceone.inventory.model.cluster.cluster.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class ClusterManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        cluster_conn: ClusterConnector = self.locator.get_connector(
        ...
        try:
            ...
            collected_cloud_services.append(
                ClusterResponse({"resource": cluster_resource})
            )
            ...
            error_response = self.generate_resource_error_response(
                e, "Cluster", "Cluster", cluster_name
            )
        ...
        for node in list_node:
            ...
            for node_condition in node_conditions:
                if node_condition.get("type", "") == "Ready":
                    cluster_node_ready_status.append(
                        node_condition.get("status", "False")
                    )
