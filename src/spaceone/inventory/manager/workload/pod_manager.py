...
class PodManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        pod_conn: PodConnector = self.locator.get_connector(
        ...
        for pod in list_all_pod:
            try:
                ...
                raw_data["restarts"] = self._get_restarts(
                    raw_data.get("status", {}).get("container_statuses", [])
                )
                raw_data["age"] = self.get_age(
                    raw_data.get("metadata", {}).get("creation_timestamp", "")
                )
                raw_data["containers"] = self.get_containers(
                    raw_data.get("status", {}).get("container_statuses", []),
