...
from spaceone.inventory.model.workload.deployment.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class DeploymentManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        pod_conn: DeploymentConnector = self.locator.get_connector(
        ...
        for deployment in list_all_deployment:
            try:
                ...
                collected_cloud_services.append(
                    DeploymentResponse({"resource": pod_resource})
                )
                ...
                error_response = self.generate_resource_error_response(
                    e, "WorkLoad", "Deployment", pod_name
                )
    ...
    def _get_ready(status):
        ...
        ready = (
            str(status.get("ready_replicas", 0)) + "/" + str(status.get("replicas", 0))
        )
        return ready
    ...
    def _get_pods(self, list_pods, list_replicasets, deployment_uid):
        ...
        for pod in list_pods:
            ...
            for replicaset in list_replicasets:
                ...
                if deployment_uid == replicaset_owner_reference.get(
                    "uid", ""
                ) and raw_replicaset.get("metadata", {}).get(
                    "uid", ""
                ) == pod_owner_reference.get(
                    "uid", ""
                ):
