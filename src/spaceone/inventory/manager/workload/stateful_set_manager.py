...
from spaceone.inventory.model.workload.statefulset.cloud_service_type import (
    CLOUD_SERVICE_TYPES,
)
...
class StatefulSetManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        ##################################
        stateful_set_conn: StatefulSetConnector = self.locator.get_connector(
        ...
        for stateful_set in list_all_stateful_set:
            try:
                ...
                raw_data["pods"] = self._get_pods(
                    list_all_pod, raw_readonly.get("metadata", {}).get("uid", "")
                )
                ...
                collected_cloud_services.append(
                    StatefulSetResponse({"resource": stateful_set_resource})
                )

            except Exception as e:
                _LOGGER.error(f"[collect_cloud_service] => {e}", exc_info=True)
                error_response = self.generate_resource_error_response(
                    e, "WorkLoad", "StatefulSet", stateful_set_name
                )
