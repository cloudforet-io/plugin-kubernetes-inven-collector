...
from spaceone.inventory.connector.workload.pod.pod import PodConnector
...
class PodManager(KubernetesManager):
    ...
    def collect_cloud_service(self, params):
        ...
        for pod in list_all_pod:
            try:
                ...
                raw_data.update({
                    'metadata': {
                        'annotations': self._convert_annotations(pod.to_dict()),
                        'labels': self._convert_labels(pod.to_dict())
                    },
                    'spec': {
                        'node_selector': self._convert_node_selector(pod.to_dict())
                    }
                })
                '''
    ...
    def _get_pod_raw_data(self, pod_raw_data):
