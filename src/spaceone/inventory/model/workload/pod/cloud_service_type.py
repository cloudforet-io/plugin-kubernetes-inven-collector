...
cst_pod._metadata = CloudServiceTypeMeta.set_meta(
    fields=[
        ...
        TextDyField.data_source('Pod IP', 'data.status.pod_ip', options={
            'is_optional': True
        }),
        TextDyField.data_source('Host IP', 'data.status.host_ip', options={
            'is_optional': True
        }),
        DateTimeDyField.data_source('Start Time', 'data.status.start_time', options={
            'is_optional': True
        }),
        TextDyField.data_source('Uid', 'data.uid', options={
            'is_optional': True
        }),
        TextDyField.data_source('QoS', 'data.status.qos_class', options={
            'is_optional': True
        })
    ],
