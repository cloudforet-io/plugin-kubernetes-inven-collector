...
pod_metadata_base_meta = ItemDynamicLayout.set_fields(
    "Metadata",
    root_path="data",
    fields=[
        ...
        TextDyField.data_source(
            "Deletion grace Period Seconds", "metadata.deletion_grace_period_seconds"
        ),
        DateTimeDyField.data_source(
            "Deletion Timestamp", "metadata.deletion_timestamp"
        ),
    ],
)

pod_metadata_annotations_meta = SimpleTableDynamicLayout.set_fields(
    "Annotations",
...
pod_metadata_labels_meta = SimpleTableDynamicLayout.set_fields(
    "Labels",
...
pod_metadata_meta = ListDynamicLayout.set_layouts(
    "Metadata",
    layouts=[
        pod_metadata_base_meta,
...
pod_spec_base_meta = ItemDynamicLayout.set_fields(
    "Spec",
    root_path="data.spec",
    fields=[
        ...
        TextDyField.data_source(
            "Termination Grace Period Seconds", "termination_grace_period_seconds"
        ),
        ListDyField.data_source("Tolerations", "tolerations"),
    ],
)

pod_spec_node_selector_meta = SimpleTableDynamicLayout.set_fields(
    "Node Selector",
    root_path="data.spec.node_selector",
    fields=[
        TextDyField.data_source("Key", "key"),
        TextDyField.data_source("Value", "value"),
    ],
...
pod_spec_dns_meta = ItemDynamicLayout.set_fields(
    "DNS Config",
    root_path="data.spec",
    fields=[
...
pod_spec_meta = ListDynamicLayout.set_layouts(
    "Spec", layouts=[pod_spec_base_meta, pod_spec_dns_meta, pod_spec_node_selector_meta]
)
...
pod_container_meta = SimpleTableDynamicLayout.set_fields(
    "Containers",
    root_path="data.spec.containers",
    fields=[
...
pod_container_init_meta = SimpleTableDynamicLayout.set_fields(
    "Init Containers",
    root_path="data.spec.init_containers",
    fields=[
        TextDyField.data_source("Name", "name"),
        TextDyField.data_source("Ports", "ports"),
        TextDyField.data_source("Args", "args"),
...
pod_volume_meta = ItemDynamicLayout.set_fields(
    "Volumes", root_path="data.spec.volumes", fields=[]
)

"""
Status
"""
pod_status_base_meta = ItemDynamicLayout.set_fields(
    "Status",
    root_path="data.status",
    fields=[
...
pod_status_conditions_meta = SimpleTableDynamicLayout.set_fields(
    "Conditions",
...
pod_status_container_status_meta = SimpleTableDynamicLayout.set_fields(
    "Container Statuses",
    root_path="data.status.container_statuses",
    fields=[
        TextDyField.data_source("Name", "name"),
        EnumDyField.data_source(
            "Ready",
...
pod_status_meta = ListDynamicLayout.set_layouts(
    "Status",
    layouts=[
        pod_status_base_meta,
...
pod_meta = CloudServiceMeta.set_layouts(
    [
        pod_metadata_meta,
        pod_status_meta,
        pod_spec_meta,
...
class PodResource(WorkLoadResource):
    ...
    data = ModelType(Pod)
    _metadata = ModelType(
        CloudServiceMeta, default=pod_meta, serialized_name="metadata"
    )
