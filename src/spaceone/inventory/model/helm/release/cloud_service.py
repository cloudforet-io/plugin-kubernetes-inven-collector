...
Release(Helm)
"""
release_base = ItemDynamicLayout.set_fields(
    "Helm",
    fields=[
...
release_info = ItemDynamicLayout.set_fields(
    "Release Info",
    fields=[
        TextDyField.data_source("Name", "data.data.release.name"),
        TextDyField.data_source("Version", "data.data.release.version"),
        TextDyField.data_source(
            "First Deployed", "data.data.release.info.first_deployed"
        ),
        TextDyField.data_source(
            "Last Deployed", "data.data.release.info.last_deployed"
        ),
...
release_chart = ItemDynamicLayout.set_fields(
    "Chart",
    root_path="data.data.release.chart",
    fields=[
...
class ReleaseResource(ServiceResource):
    ...
    _metadata = ModelType(
        CloudServiceMeta, default=release_meta, serialized_name="metadata"
    )
